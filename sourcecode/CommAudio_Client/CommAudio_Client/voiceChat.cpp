...
--
----------------------------------------------------------------------------------------------------------------------*/
DWORD WINAPI StartRecvVoice(LPVOID parameter) // valid ip for the voicecast group
{
	DWORD recvThread;
...
DWORD WINAPI StartSendVoice(LPVOID parameter)
{
	...
	if (StartWaveIn() == false) return false;
...
{
	WSASend(sendSocketInfo->socket, &(sendSocketInfo->datagram), 1, &(sendSocketInfo->bytesRECV), 0, &(sendSocketInfo->overlapped), RecvVoice);
	while (true)
	{
		SleepEx(INFINITE, TRUE);
	}
