...
DWORD WINAPI RecvMultiThread(LPVOID parameter)
{
	while (true)
	{
		...
		break; // remove when this is no longer an infinite loop
	}
}
...
{
	HANDLE semaBuffer = OpenSemaphore(SEMAPHORE_MODIFY_STATE, TRUE, "multiBuffer\0");
	...
	// wait for semaBuffer
	WaitForSingleObjectEx(semaBuffer, INFINITE, TRUE);
	// wait for semaPut
...
void PlayMulti(SemaFunc * semaStruct)
{
	...
	HANDLE semaBuffer = OpenSemaphore(SEMAPHORE_MODIFY_STATE, TRUE, "multiBuffer");
	...
	ReleaseSemaphore(semaPut, 1, NULL);
	// signal semaBuffer
	ReleaseSemaphore(semaBuffer, 1, NULL);
